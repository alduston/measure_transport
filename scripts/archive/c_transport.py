...
def comp_cond_kernel_transport(X_mu, Y_mu, Y_eta,  params, n_transports=50,
    ...
    iters = 0
    eps = 1
    noise_shrink_c = np.exp(np.log(eps)/(n_transports-10))
...
def run():
    ...
    elden_exp(10000, exp_name='kvl_exp_no_diff', normal=1, n_transports=3)
    #two_d_exp(ref_gen=sample_normal, target_gen= sample_elden_ring, N=5000, exp_name='elden_exp_alt2', n_transports=1,
              #slice_vals=[], plt_range=[[-1, 1], [-1, 1]], slice_range=[-1, 1], vmax=6, skip_idx=1)

...
def conditional_transport_exp(ref_gen, target_gen, N = 1000, n_iter = 1001, slice_vals = [], vmax = None,
     ...
     if not len(plot_idx):
        plot_idx = torch.tensor([2,3]).long()
...
def run():
    ...
    mu_vecs  = [np.random.random(d) for i in range(n_mixtures)]

    target_gen = lambda N: normalize(sample_mixtures(N, mu_vecs, sigma_vecs))

    #target_gen = lambda N: normalize(get_cond_VL_data(N, Yd = 4))
    conditional_transport_exp(ref_gen, target_gen, N=1000, n_iter=1001, slice_vals=[],
                              exp_name='nd_mixtures', plt_range=[[-5,5], [-5,5]], slice_range=[],

...
class CondTransportKernel(nn.Module):
    def __init__(self, base_params, device=None):
        ...
        self.Y_approx = geq_1d(torch.tensor(base_params['Y_mu_noisy'], device=self.device, dtype=self.dtype))
        #print(torch.linalg.norm(self.Y_mu_noisy - self.Y_eta))
...
def run():
    two_d_exp(ref_gen=sample_normal, target_gen=sample_elden_ring , N=10000, exp_name='exp', n_transports=90,
              ...
              skip_idx=1, N_plot=20000, plot_steps=True, normal=True, bins=100, var_eps=1/10)

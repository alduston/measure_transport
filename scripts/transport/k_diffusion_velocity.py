...
class Comp_transport_model:
    ...
    def map_batch(self, x_mu, y_mean, y_var, Lambda_mean, X_mean, X_var,
        ...
        z_var = self.map_var(x_mu, y_v, y_mean, Lambda_var, X_var, y_var, var_kernel, var_eps)
        z = z_mean + z_var
...
class CondTransportKernel(nn.Module):
    def __init__(self, base_params, device=None):
        ...
        self.X_var = torch.concat([self.X_mu, self.Y_velocity, self.Y_mean + self.Y_var], dim=1)
    ...
    def map_var(self, x_mu, y_v, y_mean, y_var):
        x_var = torch.concat([x_mu, y_v, y_mean + y_var], dim=1)
    ...
    def map_batch(self, x_mu, y_velocity, y_mean=0, y_var=0):
        ...
        z_var = self.map_var(x_mu, y_velocity, y_mean, y_var)
        z = z_mean + z_var
        y_approx = y_mean + y_var

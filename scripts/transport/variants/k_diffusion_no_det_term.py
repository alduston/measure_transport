...
class Comp_transport_model:
    ...
    def map_batch(self, x_mu, y_var, X_var, fit_kernel, Lambda_var, var_eps, y_eta):
        z_var = self.map_var(x_mu, y_eta, Lambda_var, X_var, y_var, fit_kernel, var_eps)
        z =  z_var

        y_approx = y_var
        batch_dict = {'y_eta': y_eta, 'y_var': y_var + z_var, 'x_mu': x_mu,
    ...
    def map_step(self, step_idx, param_dict):
        ...
        for batch_idx in batch_idxs:
            batch_x_mu, batch_y_eta,  batch_y_var = x_mu[batch_idx], y_eta[batch_idx], y_var[batch_idx]
            batch_dict = self.map_batch(batch_x_mu, batch_y_var, X_var, fit_kernel, Lambda_var, var_eps, batch_y_eta)
    ...
    def iterated_map(self, x, y, no_x=False):
        param_dict = {'y_eta': y,  'y_var': 0 * deepcopy(y),'x_mu': x, 'y_approx': deepcopy(y),
                      'y': np.concatenate([geq_1d(x, True), geq_1d(y, True)], axis=1)}
        self.approx = False
        for step_idx in range(len(self.submodel_params['Lambda_var'])):
...
class CondTransportKernel(nn.Module):
    def __init__(self, base_params, device=None):
        ...
        self.X_var = torch.concat([self.X_mu, self.var_eps * flip(self.Y_eta), self.Y_var], dim=1)

        self.Nx = len(self.X_var)
        self.Ny = len(self.Y_target)

        self.params['fit_kernel_params']['l'] *= l_scale(torch.concat([self.X_mu, self.Y_var], dim=1)).cpu()
    ...
    def map_var(self, x_mu, y_eta, y_var):
        x_var = torch.concat([x_mu, self.var_eps * flip(y_eta), y_var], dim=1)
        Lambda_var = self.get_Lambda_var()
        z_var = self.fit_kernel(self.X_var, x_var).T @ Lambda_var
        return z_var


    def map(self, X_mu, Y_eta, Y_var):
        if not len(Y_var):
            Y_var = deepcopy(Y_eta)
    ...
    def map_batch(self, x_mu, y_eta, y_var=0):
        ...
        z_var = self.map_var(x_mu, y_eta, y_var)
        z = z_var

        y_approx = y_var
        return_dict = {'y_eta': y_eta, 'y_var': y_var + z_var,
    ...
    def loss_mmd(self):
        Y_approx = self.Y_var + self.Z_var
    ...
    def loss_reg(self):
        ...
        return  self.reg_lambda * (reg_var)
    ...
    def loss_test(self):
        ...
        map_vec = self.map(x_mu, y_eta, y_var)['y']
...
def cond_kernel_transport(X_mu, Y_mu, Y_eta, Y_var, X_mu_test, Y_eta_test, Y_mu_test, X_mu_val,
                          Y_var_test, Y_noise, params, iters=-1, approx=False, mmd_lambda=0, step_num = 1,
...
def comp_cond_kernel_transport(X_mu, Y_mu, Y_eta, Y_eta_test, X_mu_test, Y_mu_test, X_mu_val, params,
    ...
    param_keys = ['fit_kernel',   'Lambda_var', 'X_var', 'var_eps']
    models_param_dict = {key: [] for key in param_keys}

    Y_var = deepcopy(Y_eta)
    Y_var_test = deepcopy(Y_eta_test)
    iters = 0
    mmd_lambda = 0
    Y_noise = Y_eta
    step_num = 1
    ...
    for i in range(n_transports):
        model, loss_dict = cond_kernel_transport(X_mu, Y_mu, Y_eta, Y_var, X_mu_test, Y_eta_test, Y_mu_test,
                                     X_mu_val,  Y_var_test, Y_noise, n_iter = n_iter, params=params,
...
def run():
    test_panel(test_name = 'no_det', N = 50, N_plot=50, n_transports=5)

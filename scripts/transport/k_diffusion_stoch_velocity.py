...
class Comp_transport_model:
    ...
    def map_var(self, x_mu, y_v, y_eta, y_mean, Lambda_var, X_var, y_var, var_kernel, var_eps):
        x_var = torch.concat([x_mu, y_v,  var_eps * flip(y_eta), y_mean + y_var], dim=1)
    ...
    def map_batch(self, x_mu, y_mean, y_var, Lambda_mean, X_mean, X_var,
        ...
        batch_dict = {'y_eta': y_eta, 'y_mean': y_mean + z_mean, 'y_var': y_var + z_var, 'x_mu': x_mu,
                      'y_approx': y_approx + z, 'y': torch.concat([x_mu, y_approx + z], dim=1), 'y_v': z}
        return batch_dict
    ...
    def map_step(self, step_idx, param_dict):
        ...
        y_v = geq_1d(torch.tensor(param_dict['y_v'], device=self.device, dtype=self.dtype))
        ...
        batch_idxs = [torch.tensor(list(range((j * batch_size), min((j + 1) * batch_size, N)))).long()
                      for j in range(1 + N // batch_size)]
        for batch_idx in batch_idxs:
            batch_x_mu, batch_y_v, batch_y_mean, batch_y_var, batch_y_eta  = x_mu[batch_idx], y_v[batch_idx], \
                                                                 y_mean[batch_idx], y_var[batch_idx], y_eta[batch_idx]
            batch_dict = self.map_batch(batch_x_mu, batch_y_mean, batch_y_var, Lambda_mean, X_mean,
                                        X_var, fit_kernel, var_kernel, Lambda_var, var_eps, batch_y_v, batch_y_eta)
    ...
    def iterated_map(self, x, y, no_x=False):
        param_dict = {'y_eta': y, 'y_mean': deepcopy(y), 'y_var': 0 * deepcopy(y),
                      'x_mu': x, 'y_approx': deepcopy(y), 'y_v': 0 * deepcopy(y),
...
class CondTransportKernel(nn.Module):
    def __init__(self, base_params, device=None):
        ...
        self.Y_velocity = geq_1d(torch.tensor(base_params['Y_velocity'], device=self.device, dtype=self.dtype))
        ...
        self.Y_velocity_test = geq_1d(torch.tensor(base_params['Y_velocity_test'], device=self.device, dtype=self.dtype))
    ...
    def map_var(self, x_mu, y_v, y_mean, y_var, y_eta):
        x_var = torch.concat([x_mu, y_v, self.var_eps * flip(y_eta), y_mean + y_var], dim=1)
        Lambda_var = self.get_Lambda_var()
        z_var = self.var_kernel(self.X_var, x_var).T @ Lambda_var
        return z_var


    def map(self, X_mu, Y_eta, Y_velocity = [], Y_mean = [], Y_var = []):
        if not len(Y_mean):
            Y_mean = deepcopy(Y_eta)
            Y_var = 0 * deepcopy(Y_mean)
            Y_velocity = 0 * deepcopy(Y_mean)
        ...
        batch_idxs = [torch.tensor(list(range((j * batch_size), min((j + 1) * batch_size, N)))).long()
                      for j in range(1 + N // batch_size)]
        for batch_idx in batch_idxs:
            x_mu ,y_velocity, y_mean ,y_var, y_eta = X_mu[batch_idx], Y_velocity[batch_idx],\
                                               Y_mean[batch_idx], Y_var[batch_idx], Y_eta[batch_idx]
            batch_dict = self.map_batch(x_mu, y_velocity, y_eta, y_mean, y_var)
            map_dict = concat_dicts(map_dict, batch_dict)
        return map_dict


    def map_batch(self, x_mu, y_velocity, y_eta, y_mean=0, y_var=0):
        ...
        z = z_mean + z_var
        y_approx = y_mean + y_var
        return_dict = {'y_velocity': z, 'y_mean': y_mean + z_mean, 'y_var': y_var + z_var,
    ...
    def loss_test(self):
        ...
        y_velocity = self.Y_velocity_test
        y_mean = self.Y_mean_test
        y_var = self.Y_var_test
        target = self.Y_test

        map_vec = self.map(x_mu, y_eta, y_velocity, y_mean, y_var)['y']
...
def cond_kernel_transport(X_mu, Y_mu, Y_eta, Y_mean, Y_var, X_mu_test, Y_eta_test, Y_mu_test, X_mu_val,
                          Y_mean_test, Y_var_test, Y_noise,Y_velocity, Y_velocity_test,
    ...
    transport_params = {'X_mu': X_mu, 'Y_mu': Y_mu, 'Y_eta': Y_eta, 'nugget': 1e-4, 'Y_var': Y_var, 'Y_mean': Y_mean,
                        ...
                        'Y_velocity':  Y_velocity, 'Y_velocity_test':  Y_velocity_test}
...
def comp_cond_kernel_transport(X_mu, Y_mu, Y_eta, Y_eta_test, X_mu_test, Y_mu_test, X_mu_val, params,
    ...
    Y_velocity_test = 0 * deepcopy(Y_mean_test)
    ...
    for i in range(n_transports):
        ...
        Y_velocity = model.Z_mean + model.Z_var

        test_map_dict = model.map(X_mu_val, Y_eta_test, Y_velocity_test, Y_mean_test, Y_var_test)
        Y_mean_test, Y_var_test = test_map_dict['y_mean'], test_map_dict['y_var']
        Y_velocity_test = test_map_dict['y_velocity']
        if approx_path:
...
def two_d_exp(ref_gen, target_gen, N=5000, plt_range=None, process_funcs=[], normal = True,
    ...
    trained_models, idx_dict = exp_func(ref_gen, normal_target_gen, N=N, vmax=vmax,N_plot=N_plot,
            ...
            while  fail_count <= 2:
                try:
                    two_d_exp(ref_gen=sample_normal, target_gen=mgan2, N=N, exp_name=f'/{test_name}/mgan2_{i_str}',
                              ...
                              bins=100,var_eps=1/2, approx_path = approx_path,  cond =cond)
                    fail_count += 3
                except torch._C._LinAlgError:
                    fail_count += 1
            ...
            while fail_count <= 2:
                try:
                    two_d_exp(ref_gen=sample_normal, target_gen=sample_spirals, N=N, exp_name=f'/{test_name}/spiral_{i_str}',
                              ...
                              var_eps=1/8, approx_path = approx_path, cond =cond)
                    fail_count +=3
                except torch._C._LinAlgError:
                    fail_count += 1
            ...
            while  fail_count <= 2:
                try:
                    two_d_exp(ref_gen=sample_normal, target_gen=sample_elden_ring, N=N, exp_name=f'/{test_name}/elden_{i_str}',
                              ...
                              slice_range=[-1.5, 1.5], vmax=8, skip_idx=1, N_plot=N_plot, plot_steps=plot_steps, normal=True,
                              bins=100, var_eps=1/12, approx_path = approx_path,  cond =cond)
                    fail_count +=3
                except torch._C._LinAlgError:
                    fail_count += 1
...
def run():
    ...
    #print('here')
    #test_panel(test_name='n_cond_exp_idk', test_keys=['checker'],
               #cond=True, N=1000, n_transports=70, plot_steps=True, N_plot=10000)

    #test_panel(test_name='cond_exp_idx2', test_keys=['checker'],
               #cond=False, N=1000, n_transports=70, plot_steps=True, N_plot=1000)

    #test_panel(test_name='n_cond_exp_big_idk', test_keys=['checker'],
               #cond=False, N=10000, n_transports=70, plot_steps=True)
    #test_panel(test_name='cond_exp_big_idk', test_keys=['checker'],

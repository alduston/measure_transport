...
class VAETransportKernel(nn.Module):
    ...
    def init_Z(self):
        ...
        ly = l_scale(self.Y)
        sig =  .5 * ly * torch.stack([sig_base for i in range(N)])
...
def run():
    ...
    VAE_transport_exp(ref_gen, target_gen, N=3000, t_iter=6005,
                              exp_name='banana_VAE_exp1', params=exp_params, plt_range=range)

    transport_exp(ref_gen, target_gen, N=3000, t_iter=6005,

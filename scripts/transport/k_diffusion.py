...
def cond_kernel_transport(X_mu, Y_mu, Y_eta, Y_mean, Y_var, X_mu_test, Y_eta_test, Y_mu_test,
    ...
    transport_params = {'X_mu': X_mu, 'Y_mu': Y_mu, 'Y_eta': Y_eta, 'nugget': 1e-4, 'Y_var': Y_var, 'Y_mean': Y_mean,
                        ...
                        'print_freq': 1000, 'learning_rate': .001, 'reg_lambda': reg_lambda, 'var_eps': var_eps,
    ...
    #n_iter = max(n_iter, n_iter * target_eps)
...
def comp_cond_kernel_transport(X_mu, Y_mu, Y_eta, Y_eta_test, X_mu_test, Y_mu_test, params,
                               final_eps=1e-6, n_transports=50, reg_lambda=1e-6, n_iter = 125,var_eps = .1):
...
def train_cond_transport(ref_gen, target_gen, params, N = 4000,  process_funcs=[],final_eps=1e-6,var_eps = .1,
...
def run():
   ...
   target_gen = sample_elden_ring
   for i in range(10):
        two_d_exp(ref_gen=sample_normal, target_gen = target_gen, N = 4000, exp_name='exp', n_transports=60,
        slice_vals=[], plt_range= [[-1,1],[-1.05,1.05]], slice_range=[-3, 3], vmax=8, skip_idx=1,
        N_plot= 2000, plot_steps = False, normal = True, bins=100, var_eps = .1)

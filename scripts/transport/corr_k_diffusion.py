...
class CondTransportKernel(nn.Module):
    ...
    def loss_inv(self):
        ...
        denoised_map_vec = torch.concat([self.X_mu, denoised_Y_approx], dim=1)
...
def comp_cond_kernel_transport(X_mu, Y_mu, Y_eta, Y_eta_test, X_mu_test, Y_mu_test, X_mu_val, Y_mu_val, params,
    ...
    for i in range(n_transports):
        model, loss_dict = cond_kernel_transport(X_mu, Y_mu, Y_eta, Y_mean, Y_var, X_mu_test, Y_eta_test, Y_mu_test,
                                     ...
                                     target_eps = target_eps, iters=iters, step_num = (5*i) + 1)
...
def run():
    test_panel(N=6000, n_transports=100, k=2, approx_path=False, test_name='test2', test_keys=['elden'])

...
def conditional_transport_exp(ref_gen, target_gen, N, t_iter = 801,exp_name= 'exp',
                              params = {'fit': {}, 'mmd': {}}, plt_range = None):
    save_dir = f'../../data/kernel_transport/{exp_name}'
    try:
        os.mkdir(save_dir)
    except OSError:
    ...
    for i,csample in enumerate(slice_samples):
        ...
        plt.hist(csample.detach().cpu().numpy(), label = f'z = {slice_vals[i]}', bins = 60, range=[-1.25, 1.25])
    plt.legend()

...
class CondTransportKernel(nn.Module):
    ...
    def init_Z(self):
        return deepcopy(self.Y_target - self.Y_ref)
...
def conditional_transport_exp(ref_gen, target_gen, N, t_iter = 501):
    ...
    train_kernel(cond_transport_kernel, n_iter= 3 * t_iter)
    ...
    sample_scatter(slice_sample, 'slice_sample.png', bins=20, d=2, range=[[-3,3],[0,10]])
    sample_hmap(slice_sample, 'slice_sample_map.png', bins=25, d=2, range=[[-3,3],[0,10]])

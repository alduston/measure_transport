...
class Comp_transport_model:
    ...
    def param_map(self, y_eta, step_idx,y_approx = [], x_mu = []):
        ...
        Lambda_mean = torch.tensor(self.submodel_params['Lambda_mean'][step_idx],device=self.device, dtype=self.dtype)
        ...
        w = torch.concat([x_mu, y_approx], dim=1)  ############
...
class CondTransportKernel(nn.Module):
    def __init__(self, base_params, device=None):
        ...
        self.X = torch.concat([self.X_mu, self.Y_approx], dim=1) ##########


        #plt.hist(self.Y_mu.detach().cpu().numpy())
        #plt.savefig('Yt_hist2.png')
        #clear_plt()
...
def run():
    ...
    two_d_exp(ref_gen, target_gen, N, n_iter=201, plt_range=[[-2.5,2.5],[-1.1,1.1]], process_funcs=[],
              slice_vals=[-1,0,1], slice_range=[-1.5,1.5], exp_name='exp', skip_idx=1, vmax=None, n_transports=12)
